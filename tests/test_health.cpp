...
{
protected:
    ConfiguredHealthTest()
        : config("..")
    {
        HealthPolicy::configure(&config);
    }
...
TEST_F(ConfiguredHealthTest, MessageDropDegradationValue)
{
    EXPECT_EQ(
        4,
        HealthPolicy::degradation(FaultType::MessageDrop, 1));
}

TEST_F(ConfiguredHealthTest, DelayDegradationValue)
{
    EXPECT_EQ(
        2,
        HealthPolicy::degradation(FaultType::TransmissionDelay, 1));
}

TEST_F(ConfiguredHealthTest, TimeoutDegradationValue)
{
    EXPECT_EQ(
        6,
        HealthPolicy::degradation(FaultType::Timeout, 1));
}

TEST_F(ConfiguredHealthTest, InvalidPayloadDegradationValue)
{
    EXPECT_EQ(
        3,
        HealthPolicy::degradation(FaultType::InvalidPayload, 1));
}

TEST_F(ConfiguredHealthTest, TransmissionFailureDegradationValue)
{
    EXPECT_EQ(
        7,
        HealthPolicy::degradation(FaultType::TransmissionFailure, 1));
}

TEST_F(ConfiguredHealthTest, EcuOfflineDegradationValue)
{
    EXPECT_EQ(
        9,
        HealthPolicy::degradation(FaultType::EcuOffline, 1));
}

TEST_F(ConfiguredHealthTest, HealthRecoveryDoesNotExceed100)
{
    EXPECT_EQ(100, HealthPolicy::recover(95));
    EXPECT_EQ(100, HealthPolicy::recover(99));
    EXPECT_EQ(100, HealthPolicy::recover(100));
}
